...
public class ChessGui extends JFrame {
	...
	private static ArrayList<JLabel> moveOptions;
	private static Piece pieceHolder;
	private static Piece whiteKing;
	private static Piece blackKing;
	private static ArrayList<Coordinate> currMoveOptions;
	private static HashMap<Coordinate, Piece> map;
	private static JLabel[] capturedLabels;
	...
	private void generatePieces() {
		// initializes the necessary arrays and maps needed to store each piece's location
		// throughout the game
		pieces = new ArrayList<Piece>();
		map = new HashMap<Coordinate, Piece>();
		moveOptions = new ArrayList<JLabel>();
		pieceHolder = null;
		// creates an array of pieces containing all of the pieces that start in a game of chess
		Piece[][] pieceArray = new Piece[][] { 
			{new King(new Coordinate(4, 0), Colour.WHITE), new Queen(new Coordinate(3, 0), Colour.WHITE), new Rook(new Coordinate(0, 0), Colour.WHITE), 
				new Rook(new Coordinate(7, 0), Colour.WHITE), new Knight(new Coordinate(1, 0), Colour.WHITE), new Knight(new Coordinate(6, 0), Colour.WHITE),
				new Bishop(new Coordinate(2, 0), Colour.WHITE), new Bishop(new Coordinate(5, 0), Colour.WHITE), new Pawn(new Coordinate(0, 1), Colour.WHITE),
				new Pawn(new Coordinate(1, 1), Colour.WHITE), new Pawn(new Coordinate(2, 1), Colour.WHITE), new Pawn(new Coordinate(3, 1), Colour.WHITE),
				new Pawn(new Coordinate(4, 1), Colour.WHITE), new Pawn(new Coordinate(5, 1), Colour.WHITE), new Pawn(new Coordinate(6, 1), Colour.WHITE),
				new Pawn(new Coordinate(7, 1), Colour.WHITE)}, 
			{new King(new Coordinate(4, 7), Colour.BLACK), new Queen(new Coordinate(3, 7), Colour.BLACK), new Rook(new Coordinate(0, 7), Colour.BLACK),
				new Rook(new Coordinate(7, 7), Colour.BLACK), new Knight(new Coordinate(1, 7), Colour.BLACK), new Knight(new Coordinate(6, 7), Colour.BLACK),
				new Bishop(new Coordinate(2, 7), Colour.BLACK), new Bishop(new Coordinate(5, 7), Colour.BLACK), new Pawn(new Coordinate(0, 6), Colour.BLACK),
				new Pawn(new Coordinate(1, 6), Colour.BLACK), new Pawn(new Coordinate(2, 6), Colour.BLACK), new Pawn(new Coordinate(3, 6), Colour.BLACK),
				 new Pawn(new Coordinate(4, 6), Colour.BLACK), new Pawn(new Coordinate(5, 6), Colour.BLACK), new Pawn(new Coordinate(6, 6), Colour.BLACK),
				 new Pawn(new Coordinate(7, 6), Colour.BLACK),}
			};
	...
	private void createinfoPanel() {
		// creates the infoPanel, which holds all of the information going on in the game
		...
		// creates the label holding all of the information regarding what black pieces remain
		JLabel lblBlackRemaining = new JLabel("<html>Black Remaining:<br>Pawns: 8<br>Knights: 2<br>Bishops: 2<br>"
				+ "Rooks: 2<br>Queens: 1</html>");
		...
		// creates the panel for holding all of the information concerning the white pieces
		JPanel WhitePanel = new JPanel();
		WhitePanel.setFocusable(false);
		infoPanel.add(WhitePanel);
		WhitePanel.setLayout(null);
		
		// creates the label holding all of the information regarding which black pieces remain
		JLabel lblWhiteRemaining = new JLabel("<html>White Remaining:<br>Pawns: 8<br>Knights: 2<br>Bishops: 2<br>" + 
				"Rooks: 2<br>Queens: 1</html>");
		lblWhiteRemaining.setBounds(0, 136, 102, 125);
		lblWhiteRemaining.setFocusable(false);
		WhitePanel.add(lblWhiteRemaining);
		
		// initializes an array to store the two labels for remaining pieces so they can be
	...
	/**
	 * Creates the action listeners for each button on the chess board. Performs many actions for the chess
	 * game based upon the Piece class and various helper methods.
	 */
	...
	/**
	 * Takes in a button and returns the respective coordinate that the button represents.
	...
	/**
	 * Getter method for the pieces ArrayList<Piece> holding all of the pieces remaining in the game
	...
	/**
	 * Getter method for the map HashMap<Coordinate, Piece> holding all of the coordinates that have
	 * been mapped to pieces
	 * @return map of type HashMap<Coordinate, Piece>
	 */
	public static HashMap<Coordinate, Piece> getMap() {
		return map;
	}
	
	/**
	 * Getter method for the enPassant Coordinate holding the coordinate that can be taken next
	 * turn by a pawn through en passant.
	...
	/**
	 * Checks to see if the passed coordinate project is threatened by any enemy pieces.
	 * @param coor The coordinate to check for being threatened
	 * @return True if the passed coordinate is threatened, false if not
	 */
	public static boolean isCoorThreatened(Coordinate coor) {
	...
	/**
	 * Moves the currently selected piece to the passed coordinate related to the passed button.
	 * @param coor The coordinate that the currently selected piece should be moved to
	...
	/**
	 * Generates JLabels to indicate each move option that the currently selected piece has.
	 */
	private void generateMoveOptions() {
		int i = 0;
		// gets the current move options and verifies that none of them put the king in check
		currMoveOptions = (ArrayList<Coordinate>) pieceHolder.canMove().clone();
		verifyMoveOptions(currMoveOptions);
		// adds icons for each legal move option on the board to show where the currently
	...
	/**
	 * Checks to make sure that each move that each move of the current piece does not
	 * result in the king being threatened. Removes all moves that do this from being
	 * legal moves.
	 */
	 ...
	 * @param oldPiece The piece that was in the position the selected piece moved to
	...
	/**
	 * Rotates the board 180 degrees whenever the turn is swapped, so the
	 * current player can view from their own perspective.
	 */
	private void rotateScreen() {
		for(JButton[] btns: buttons) {
			for(JButton btn: btns) {
				Rectangle rect = btn.getBounds();
				btn.setBounds((int)rect.getX(), (buttonSize * 7) - (int)rect.getY(), 
		...
		// board angle
		capturedLabels[Math.abs(turnColor - 1)].setText("<html>White Remaining:<br> Pawns: " +
				piecesRemaining[0][0] + "<br> Knights: " + piecesRemaining[0][1] + "<br> Bishops: " +
				piecesRemaining[0][2] + "<br> Rooks: " + piecesRemaining[0][3] + "<br> Queens: " + 
				piecesRemaining[0][4]);
		
		capturedLabels[turnColor].setText("<html>Black Remaining:<br> Pawns: " +
				piecesRemaining[1][0] + "<br> Knights: " + piecesRemaining[1][1] + "<br> Bishops: " +
				piecesRemaining[1][2] + "<br> Rooks: " + piecesRemaining[1][3] + "<br> Queens: " + 
				piecesRemaining[1][4]);
	}
	
	/**
	 * Checks to see if a pawn was moved onto the back row. If so, a promotion prompt appears, and
	 * the player is asked if they want to promote to a knight, bishop, rook, or queen.
	 */
	private void checkForPromotion() {
		...
		// checks to make sure that the pawn moved to the correct position in order to be promoted
		...
		// creates an array of possible pieces for promotion to show on the prompt's labels
		Icon[] icons = new Icon[4];
		Piece[] possiblePromos = {
				new Knight(pieceHolder.getLocation(), pieceHolder.getColour()),
				new Bishop(pieceHolder.getLocation(), pieceHolder.getColour()),
				new Rook(pieceHolder.getLocation(), pieceHolder.getColour()),
				new Queen(pieceHolder.getLocation(), pieceHolder.getColour())
						};
		// gets each possible promo's icon in an array of icons to show on the input dialog
		for(int i = 0; i < 4; i++) {
		...
		// adds the newly promoted piece to the pieces array with the pawn's old location, and removes
	...
	/**
	 * Checks to see if the last move taken by the king was a castle, then performs the proper
	 * move for the corresponding rook.
	 */
	private void checkForCastle() {
		// the last move cannot be a castle if the piece used wasn't a king and
	...
	/**
	 * Checks if the most recent move was an en passant. If it was, deletes the piece that was
	 * "passed" by the en passant action.
	 */
	private void checkForEnPassant() {
		// enPassant cannot happen with non-pawn pieces, and also cannot happen if a pawn didn't move
	...
	/**
	 * If a pawn ever moves two squares, this method holds the coordinate it hopped over in the enPassant
	 * field. Then if a pawn tries to capture that certain square the next turn, it will be allowed.
	 */
	private void allowForEnPassant() {
		...
		// the enPassant coordinate to the coordinate that can be captured by other pawns
		// for the next turn.
	...
	private void checkForMates() {
		...
		String message = checkForCheck() ? "<html>Checkmate! " + turn.toString() + " Wins!" : "<html>Stalemate! It's a Draw!";
		...
		promoPanel.setBounds(100, 250, 200, 500);
		@SuppressWarnings("static-access")
		int returnValue = promoPanel.showOptionDialog(contentPane, message + "<br>Would you like to play again?</html>", "Game Over!",
				JOptionPane.DEFAULT_OPTION,  JOptionPane.QUESTION_MESSAGE, getOtherKing().getIcon(), options, options[0]);
		
		// assumes that the user wants to close the program if the prompt window is closed
		if(returnValue == JOptionPane.CLOSED_OPTION) {
			returnValue = 1;
		}
	...
	/**
	 * Swaps the turn along with the turn label based on which turn is currently going. Also
	 * adds the check notification to the turnLabel if the current player is in check.
	 */
	private void swapTurn() {
		...
		// changes the information label based on whether or not the current player is in check.
		if(checkForCheck()) {
	...
	/**
	 * Changes the captured labels by decrementing the type of the passed piece by one, then
	 * incrementing the type of the other passed piece by one in the case of a promotion.
	...
	private void changeCapturedLabels(Piece pieceToRemove, int promotion) {
		...
		// adds one piece to the counter of the piece is the piece was a result of promotion
		if(promotion != -1) {
			piecesRemaining[color][promotion]++;
		}
		
		// sets the new captured label based upon the changes made above
		capturedLabels[color].setText("<html>" + pieceToRemove.getColour().toString() + " Remaining:<br> Pawns: " +
				piecesRemaining[color][0] + "<br> Knights: " + piecesRemaining[color][1] + "<br> Bishops: " +
				piecesRemaining[color][2] + "<br> Rooks: " + piecesRemaining[color][3] + "<br> Queens: " + 
				piecesRemaining[color][4]);
	}
}